        ...
        let layout = UICollectionViewCompositionalLayout { _, _ in
            ...
            let item = NSCollectionLayoutItem(
                layoutSize: NSCollectionLayoutSize(
                    ...
                    heightDimension: .estimated(0) // 카드 크기: 내부 크기에 따라 늘어남 (아이템-그룹 같아야 함)
                )
            )
            ...
            section.contentInsets = .init(top: 32, leading: 20, bottom: 80, trailing: 0) // CollectionBackgroundView 크기 (안의 view가 아님)
            
            
            // header
            section.boundarySupplementaryItems = [
                NSCollectionLayoutBoundarySupplementaryItem(
                    layoutSize: .init(widthDimension: .fractionalWidth(1), heightDimension: .absolute(48)), // 헤더 크기
...
extension HomeViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row % 2 == 0 {
            ...
            cell.item = Item(title: "제목", content: "홀수 내용입니다.")
        }
